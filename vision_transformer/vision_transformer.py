...
transform = Compose([Resize((144, 144)), ToTensor()])

# Load dataset
dataset = ImageFolder(root=r".\vision_transformer\data\train", transform=transform)
...
cm = confusion_matrix(outputs.argmax(-1).cpu(), labels.cpu())
...
plot_validation_loss(epoch_losses)